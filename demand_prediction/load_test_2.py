...
model = RNN()
model.load_state_dict(torch.load('params_without_filter.pkl'))

dispatcher_data = np.load("K:\毕业论文\TaxiData\demand\\dispatcher_data_11_05_region_823.npy").tolist()
...
for t_step, (t_x, t_y) in enumerate(test_loader):
    ...
    print_out = 'test MAPE: %.4f' % Evaluate.MAPE(all_pred_y, all_test_y) + \
