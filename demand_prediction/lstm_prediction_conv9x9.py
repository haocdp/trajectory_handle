...
from demand_prediction.Demand_Conv_9x9 import Net
...
elogger = logger.Logger("demand_lstm_prediction_conv9x9")


def load_data():
    net_dataset = np.load(base_path + "/demand/net_data_9x9.npy").tolist()
...
for epoch in range(EPOCH):
    for step, (b_x, b_y) in enumerate(loader):  # gives batch data
        ...
        b_x = b_x.view(-1, TIME_STEP, 85)  # reshape x to (batch, time_step, input_size)
