...
def load_data():
    ...
    for trajectory, label in trajectories:
        ...
        if c < count:
            if len(new_tra) >= 20:
                train_data.append(new_tra[:20])
                train_labels.append(label)
            else:
                for i in range(0, 20 - len(new_tra)):
                    new_tra.append(new_tra[-1])
                train_data.append(new_tra)
                train_labels.append(label)
        else:
            if len(new_tra) >= 20:
                test_data.append(new_tra[:20])
                test_labels.append(label)
            else:
                for i in range(0, 20 - len(new_tra)):
                    new_tra.append(new_tra[-1])
                test_data.append(new_tra)
                test_labels.append(label)
        c += 1
        ...
        for vector in x:
            for item in vector:
                if new_vector is None:
                    ...
                    new_vector = torch.cat((new_vector, self.poi_embeds(torch.cuda.LongTensor([item[2].item()]))[0]))
        x = new_vector.view(-1, TRA_LENGTH, 30)
        if gpu_avaliable:
            x = x.cuda()
...
for epoch in range(EPOCH):
    for step, (b_x, b_y) in enumerate(loader):  # gives batch data
        ...
        b_x = b_x.view(-1, TRA_LENGTH, 3)  # reshape x to (batch, time_step, input_size)
...
test_output = rnn(test_data[:10].view(-1, TRA_LENGTH, 3))
if gpu_avaliable:
    pred_y = torch.max(test_output, 1)[1].cuda().data
else:
    pred_y = torch.max(test_output, 1)[1].data.numpy()
