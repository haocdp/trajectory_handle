...
def load_data():
    ...
    for trajectory, label in trajectories:
        ...
        if len(new_tra) < 10 or len(new_tra) > 30:
            c += 1
            continue
        if c < count:
...
for epoch in range(EPOCH):
    for step, b_x in enumerate(train_data):  # gives batch data
        b_x = torch.FloatTensor(b_x).view(1, -1, 3)  # reshape x to (batch, time_step, input_size)

        output = train(b_x)
        loss = loss_func(output, torch.LongTensor([train_labels[step]]))  # cross entropy loss
        ...
        if step % 3200 == 0:
            pred_y = []
            for i, t_x in enumerate(test_data):
                t_x = torch.FloatTensor(t_x).view(1, -1, 3)
                test_output = train(t_x)
                pred_y.append(torch.max(test_output, 1)[1].data.item())
            # pred_y = torch.max(test_output, 1)[1].data.numpy()
            accuracy = float(sum(torch.LongTensor(test_labels) == torch.LongTensor(pred_y))) / float(len(test_labels))
