...
elogger = logger.Logger("CNN_LSTM_without_filter_label_is_region")
...
def load_data():
    ...
    np.save("cnn_lstm_prediction_label_is_region_car_to_ix", car_to_ix)
    np.save("cnn_lstm_prediction_label_is_region_poi_to_ix", poi_to_ix)
    np.save("cnn_lstm_prediction_label_is_region_region_to_ix", region_to_ix)
...
for epoch in range(EPOCH):
    for step, (b_x, b_y) in enumerate(loader):  # gives batch data
        ...
        b_x = b_x.view(-1, 10, 8)  # reshape x to (batch, time_step, input_size)
